    ...
    for attempt in range(retries):
        try:
            ...
            if client.is_ready():
                ...
                print(f"⚠️ Attempt {attempt+1}/{retries}: Weaviate not ready yet.")
...
def ensure_schema(client, class_name: str = "Document"):
    if client is None:
        print("⚠️ Weaviate client unavailable — cannot ensure schema.")
        return
    try:
...
def upsert_documents(client, docs, class_name: str = "Document"):
    if client is None:
        print("⚠️ Skipping Weaviate upsert (no active client).")
        return
    try:
        coll = client.collections.get(class_name)
    except Exception as e:
        print(f"⚠️ Failed to fetch collection '{class_name}': {e}")
        return
    ...
    for d in docs:
        try:
            ...
            fail_count += 1
            print(f"  ⚠️ Failed to insert document '{d.get('title','unknown')}': {e}")
    print(f"✅ Inserted {success_count} document(s), ⚠️ failed {fail_count}.")
